locals {
  ...
  cluster_name             = "YOUR_CLUSTER_NAME" # Must be unique for each Modern Gitops Stack deployment in a single AWS account. Contains only alphanumeric and hyphens.
  base_domain              = "your.domain.here"  # Must match a Route 53 zone in the AWS account where you are deploying the Modern Gitops Stack.
  subdomain                = "apps"
  ...
  # The VPC CIDR must be unique for each Modern Gitops Stack deployment in a single AWS account.
