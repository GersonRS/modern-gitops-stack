locals {
  # Parameters for the resources that are created outside this code, but still on the Azure subscription where the Modern Gitops Stack will be deployed.
  ...
  # Parameters used for this deployment of the Modern Gitops Stack.
  common_resource_group    = "YOUR_COMMON_RESOURCE_GROUP" # The resource group where the common resources will reside. Must be unique for each Modern Gitops Stack deployment in a single Azure subscription. 
  location                 = "YOUR_LOCATION"
  kubernetes_version       = "1.28"
  sku_tier                 = "Standard"
  cluster_name             = "YOUR_CLUSTER_NAME" # Must be unique for each Modern Gitops Stack deployment in a single Azure subscription.
  base_domain              = "your.domain.here"  # Must match a DNS zone in the Azure subscription where you are deploying the Modern Gitops Stack.
  subdomain                = "apps"
  ...
  # The virtual network CIDR must be unique for each Modern Gitops Stack deployment in a single Azure subscription.
  ...
  # Local containing all the OIDC definitions required by the Modern Gitops Stack modules.
  oidc = {
