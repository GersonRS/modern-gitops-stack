...
An example of a local deployment of a Kubernetes cluster on Azure AKS is provided https://github.com/GersonRS/modern-gitops-stack/tree/main/examples/aks[here]. Clone this repository and modify the files at your convenience.
...
* *`dns.tf`* - creation of the wildcard record for the ingresses of the Modern Gitops Stack components;
* *`oidc.tf`* - addition of the redirect URIs to the Azure AD Enterprise Application in order to use it to authenticate to the Modern Gitops Stack components providing a web interface;
...
* An Enterprise Application on Entra ID to use as an identity provider for the Modern Gitops Stack components;
* The Azure subscription needs to have a Key Vault to store the secrets used to pass the credentials of said application to the Modern Gitops Stack components;
...
[TIP]
====
In this repository, you will find an example of Terraform code that could provision the required resources above. You can find this code https://github.com/GersonRS/modern-gitops-stack/tree/main/examples/aks/requirements[here].
...
Check the https://github.com/GersonRS/modern-gitops-stack/tree/main/examples/aks/requirements/application.tf[`application.tf`] from the tip above and adapt the Terraform resources in order to create the application yourself.
