...
An example of a local deployment on Amazon EKS is provided https://github.com/GersonRS/modern-gitops-stack/tree/main/examples/eks[here]. Clone this repository and modify the files at your convenience.
...
* *`csi_drivers.tf`* - creation of the required resources as well as the Modern Gitops Stack modules needed for the CSI drivers of the cluster;
* *`outputs.tf`* - the output variables of the Modern Gitops Stack;
...
In this example, we use the https://aws.amazon.com/eks/[Amazon EKS] OIDC provider. We provide a xref:oidc-aws-cognito:ROOT:README.adoc[module] that takes in a Cognito pool ID and its domain to provide you with the required configuration to deploy the Modern Gitops Stack applications.
...
The `user_map` variable of that module allows you to create OIDC users used to authenticate to the Modern Gitops Stack applications. You should receive an e-mail from AWS with a temporary password to login for the first time.
...
+
IMPORTANT: The `cluster_name` and `vpc_cidr` must be unique for each Modern Gitops Stack deployment in a single AWS account and the `base_domain` must match a Route 53 zone in that same account.
+
...
The output `modern_gitops_admins` list all users and respective e-mails that were configured using the OIDC module:
...
Those users should have received an e-mail with a temporary password in order to login to the Modern Gitops Stack applications for the first time.
