...
In order to ease up the burden of the project maintainers, there is an internal Project Board on GitHub used to track the progress of the PRs and issues. The board is available only to the https://github.com/orgs/GersonRS/teams/is-modern-gitops-stack/[`@GersonRS/is-modern-gitops-stack` team] and is available https://github.com/orgs/GersonRS/projects/3/[here]. All the repositories of the Modern Gitops Stack are also connected to this project upon creation.
...
The app is called `Modern Gitops Stack Project` and is available https://github.com/apps/modern-gitops-stack-project[here].
...
IMPORTANT: The reason to not install the app on all the repositories by default was to further limit the scope of the app, although *this adds the burden of installing it on each repository manually every time a new repository of the Modern Gitops Stack is created*.
...
NOTE: It is the step _Generate authentication token from GitHub App_ that uses the GitHub app created above in order to generate a token with the proper permissions that is then passed to the _Add PR or issue to Modern Gitops Stack project board_ step.
